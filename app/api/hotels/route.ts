...
export async function GET(request: NextRequest) {
  try {
    ...
    if (includeInactive && !ownerId) {
      // Only require admin role if includeInactive is true AND it's not an owner-specific request
      await requireRole(request, 'admin')
    }
    if (ownerId) {
      // First check if user exists as partner in database (bypass auth for development)
      const { default: User } = await import('@/models/User')
      const user = await User.findOne({ uid: ownerId })
      if (!user || user.role !== 'partner') {
        return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
      }
      // User exists as partner, allow access
    }
