...
const getHandler = async (request: NextRequest) => {
  try {
    ...
    // Check if ownerId is provided - allow access for partners without full auth
    let user = null
    if (ownerId) {
      const { default: User } = await import('@/models/User')
      const userRecord = await User.findOne({ uid: ownerId })
      if (!userRecord || userRecord.role !== 'partner') {
        return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
      }
      // Create user object for partner
      user = { uid: ownerId, role: 'partner' }
    } else {
      // Require full authentication for other requests
